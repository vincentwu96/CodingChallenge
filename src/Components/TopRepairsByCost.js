...
class TopRepairsByCost extends Component {
  ...
  dataInit(maintainence_data){
    ...
    for(let i=0; i<maintainence_data.length; i++){
      ...
      data[i].pk = data[i].extended_cost_$ + data[i].labour_cost_$; // overwrite pk with total cost
    }
    data.sort((a,b) => parseFloat(a.pk) - parseFloat(b.pk));  // data sorted by total cost
  ...
  render() {
    return (
      <div className="TopRepairsByCost">
      <h3>Top Repairs By Cost</h3>
      ...
      <Tooltip/>
      <Legend />
      <Bar name="Total Cost $" dataKey="pk" fill="#000080" />
      </BarChart>
      </ResponsiveContainer>
